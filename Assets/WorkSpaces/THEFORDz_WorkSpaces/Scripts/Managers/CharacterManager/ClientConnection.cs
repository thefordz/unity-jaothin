...
public class ClientConnection : SingletonNetwork<ClientConnection>
{
    ...
    [SerializeField] private LobbyDataSO m_lobbyData;
    ...
    private void Shutdown()
    {
        if (m_lobbyData)
        {
            if (m_lobbyData.connectedLobby != null)
            {
                if (m_lobbyData.connectedLobby.HostId == m_lobbyData.playerId)
                {
                    Lobbies.Instance.DeleteLobbyAsync(m_lobbyData.connectedLobby.Id);
                }
                    
                else
                {
                    Lobbies.Instance.RemovePlayerAsync(m_lobbyData.connectedLobby.Id, m_lobbyData.playerId);
                }
            }
        }
